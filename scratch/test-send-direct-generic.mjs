...
async function runDirectTest() {
    ...
    for (const phone of PHONES) {
        ...
        if (guests && guests.length > 0) {
            ...
            console.log(`❌ Could not find guest with phone ${phone}.`);
        }
    }
    ...
    // Call the Netlify handler directly with campaignType: 'direct'
    console.log(`\n🚀 Triggering send-batch-v2 handler for ${guestIds.length} guests in 'direct' mode (lony_generic)...`);
    const reqBody = {
        httpMethod: 'POST',
        body: JSON.stringify({
            eventId: EVENT_ID,
