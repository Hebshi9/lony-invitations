  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      if (changes?.messages) {
        ...
        if (isBridgeButton) {
            console.log(`[Bridge Webhook] 🌉 Guest ${from} pressed bridge button: "${btnText}". Sending Marketing Invite...`);
            
            // 1. Find latest guest with stashed invitation payload
            ...
            const stashedGuest = stashedGuests?.[0];

            if (stashedGuest?.pending_marketing_data) {
                console.log(`[Bridge Webhook] 🚀 Re-triggering invitation for ${stashedGuest.name}`);
                const bRes = await fetch(`https://graph.facebook.com/v21.0/${META_PHONE_NUMBER_ID}/messages`, {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${META_ACCESS_TOKEN}`, 'Content-Type': 'application/json' },
                    body: JSON.stringify(stashedGuest.pending_marketing_data)
                });
                if (bRes.ok) {
                    ...
                    await supabase.from('whatsapp_messages').insert([{
                        ...
                        message_text: 'دعوة رسمية (بعد الجسر)',
                        status: 'sent',
                    ...
                    console.error(`[Bridge Webhook] ❌ Re-send failed for ${stashedGuest.name}:`, JSON.stringify(errData));
...
async function handleRSVP(phone, status, contextId = null) {
  ...
  if (!targetGuestId) {
    ...
    // Use the MOST RECENT guest record for this phone to be safe
    ...
    if (guestMatch) {
      ...
      console.log(`[Fallback Match] Found Guest ID: ${targetGuestId} for Event: ${targetEventId}`);
    }
  }
