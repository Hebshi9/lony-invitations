...
الرد يجب أن يكون JSON فقط:
{
  ...
  "location": ""
     ...
     */
    async parseBusinessEntry(text) {
        try {
            const prompt = `
أنت مساعد مالي ذكي متخصص في إدارة طلبات شركة "لوني" لدعوات الزفاف.
حلل النص التالي واستخرج البيانات المالية والتعاقدية بدقة:

"${text}"

المطلوب استخراج:
1. clientName: اسم العميل
2. clientPhone: رقم الجوال (تأكد من الصيغة)
3. serviceType: نوع الخدمة (اختر واحدة من: "بكج كامل", "تصميم فقط", "تصميم وباركود")
4. totalPrice: المبلغ الإجمالي المتفق عليه (رقماً)
5. depositAmount: مبلغ العربون المدفوع (رقماً)
6. designerFee: تكلفة المصممة إن ذكرت (رقماً)
7. followUpDate: تاريخ المتابعة القادم (بصيغة YYYY-MM-DD) - إذا لم يذكر، اقترح تاريخاً بعد 3 أيام.

الرد يجب أن يكون JSON فقط بهذا الشكل:
{
  "clientName": "",
  "clientPhone": "",
  "serviceType": "بكج كامل",
  "totalPrice": 0,
  "depositAmount": 0,
  "designerFee": 0,
  "followUpDate": ""
...
تعليمات هامة:
- لا تخترع أرقاماً. إذا لم يذكر السعر، ضعه 0.
- حول السعر إلى ريال سعودي.
- لا تضف أي نص قبل أو بعد JSON.
            `;
            ...
            console.error('[Gemini] Business entry parsing failed:', error);
            return null;
        }
    }
